...
class FormStore extends Store {
  constructor(STATE_PROPERTY) {
    ...
    this.createAction("formArrayAdd", (state, {
      ...
      const newState = state.updateIn(["form", ...key], arr => arr ? arr.insert(index, fromJS(newFields)) : List([fromJS(newFields)]));
  ...
  _processForm(fields, initialKey = [], skipRule = false) {
    ...
    const processForm = (field, key = [], skipValue = false) => {
      if (isPlainObject(field)) {
        ...
        if (!skipRule) {
          setKeyValue(this.validationRules, this._resetFormArrayKeys(key), arr);
        }
